...
<a class="dropdown-item active" href="tematy-klasy.php">Tematy klasami</a>
...
?>
	...
	<h3 class="m-4">Tematy dla moich klas</h3>

	<form method="GET">
		<div class="form-group col-sm-4 m-2">
			<select class="form-control" id="sel" name="id">
				<?php
					require "connect.php";
					$id = $_SESSION['id'];
					$sql = "SELECT klasa_id, nazwa FROM nauczyciele_i_klasy, klasy WHERE nauczyciel_id=$id AND klasa_id = id_klasy";
					
					$result = $conn -> query($sql);
					
					while ($wiersz = $result -> fetch_assoc())
					{
						echo '<option value="' . $wiersz['klasa_id'] . '" ';
						if (isset($_GET['id']))
						{
							$get_id = $_GET['id'];
							if ($wiersz['klasa_id'] == $get_id)
								echo "selected";
						}
			...
			<button type="submit" class="btn btn-primary text-center mt-4 mb-4">Pokaż tematy dla klasy</button>
		</div>
	</form>
	...
	$sql = "SELECT dostepnosc.id_tematu FROM dostepnosc, tematy WHERE grupa='$id_klasy' AND dostepnosc.id_tematu = tematy.id_tematu AND tematy.uzytkownik_id=$user_id";
...
END;
			...
			$conn -> close();
				...
				echo '<p class="m-4"> Nie utworzyłeś jeszcze żadnych tematów dla tej klasy. </p>';
