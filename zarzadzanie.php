...
<a class="dropdown-item active" href="zarzadzanie.php">Zarządzanie użytkownikami</a>
...
?>
		...
		<div class="card mx-4 my-4">
			<div class="card-header"><h3>Lista użytkowików</h3></div>
			<div class="card-body">
				<div class="table-responsive">
					<table id="users" class="table table-bordered mb-4" cellspacing="0">
						<thead>
							<tr>
								<th>Login</th>
								<th>Imię</th>
					...
					<?php
						require "connect.php";
						$sql = "SELECT id_uzytkownika, imie, nazwisko, login, email, typ_konta FROM uzytkownicy WHERE id_uzytkownika != -1";
						...
						if ($wynik -> num_rows > 0)
						{
							while ($wiersz = $wynik -> fetch_assoc())
							{
								...
								echo "<td>" . $wiersz['typ_konta'] . "</td>";
								
								echo '<td class="pr-0 pl-0 text-center">';
								echo '<button type="button" class="btn btn-primary btn-sm ml-2 mr-2 my-1 text-center" onclick="edit(' . $wiersz['id_uzytkownika'] . ')"> <i class="icon-pencil"> </i>Edytuj</button>';

								echo '<button type="button" class="btn btn-primary btn-sm ml-2 mr-2 my-1 text-center" onclick="usun(' . $wiersz['id_uzytkownika'] . ')"><i class="icon-delete"> </i>Usuń</button>';
