...
?>
		...
		<div class="card mx-4 my-4">
			...
			?>
				...
				<?php
					require "connect.php";
					$sql = "SELECT id_uzytkownika, login, imie, nazwisko, typ_konta FROM uzytkownicy WHERE czy_aktywne = 0";
...
END;
						while ($wiersz = $wynik -> fetch_assoc())
						{
							...
							if ($wiersz['typ_konta'] == "uczen")
							{
								$userId = $wiersz['id_uzytkownika'];
								$sql_kl = "SELECT nazwa FROM klasy, uczniowie_i_klasy WHERE id_klasy = klasa_id AND uczen_id = $userId";
								$resultKl = $conn -> query($sql_kl);
								$wierszKl = $resultKl -> fetch_assoc();
								$klasa = $wierszKl['nazwa'];
								
								echo "<td>" . $wiersz['login'] . "</td>";
								echo "<td>" . $wiersz['imie'] . "</td>";
								echo "<td>" . $wiersz['nazwisko'] . "</td>";
								echo "<td>" . $wiersz['typ_konta'] . " (" . $klasa . ")" . "</td>";
