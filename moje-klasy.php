<?php
	...
	if (isset($_POST['listaKlas']))
	{
		...
		$klasa = $_POST['listaKlas'];
		$id = $_SESSION['id'];
		
		$sql_add = "INSERT INTO nauczyciele_i_klasy VALUES ($id, $klasa)";
		$conn -> query($sql_add);
...
<head>
	...
	<script src="script-klasyNauczyciel.js"></script>
</head>
...
echo '<a class="dropdown-item" href="profil.php?id=' . $_SESSION['id'] . '">Mój profil</a>';
echo<<<END
<a class="dropdown-item active" href="moje-klasy.php">Moje klasy</a>
...
?>
		...
		<div class="card mx-4 my-4">
			<div class="card-header"><h3>Moje klasy</h3></div>
			<div class="card-body">
				<form method="POST">
					<div class="form-group col-sm-4">
						<select class="form-control" id="sel" name="listaKlas">
							<?php
								require "connect.php";
								$id = $_SESSION['id'];
								$sql = "SELECT id_klasy, nazwa FROM klasy WHERE id_klasy NOT IN(SELECT klasa_id FROM nauczyciele_i_klasy WHERE nauczyciel_id=$id)";
						...
						<button type="submit" class="btn btn-primary text-center mt-4">Dodaj klasę</button>
					</div>
				</form>
				...
				<div class="table-responsive">
					...
					<?php
						...
						$sql = "SELECT nazwa, id_klasy FROM klasy, nauczyciele_i_klasy WHERE klasy.id_klasy = nauczyciele_i_klasy.klasa_id AND nauczyciele_i_klasy.nauczyciel_id = $zalogowany";
						...
						if ($wynik -> num_rows > 0)
						{
							while ($wiersz = $wynik -> fetch_assoc())
							{
								...
								echo '<button type="button" class="btn btn-primary btn-sm ml-2 mr-2 my-1 text-center" onclick="usun(' . $wiersz['id_klasy'] . ", " . $_SESSION['id'] . ')"><i class="icon-delete"> </i>Usuń</button>';
